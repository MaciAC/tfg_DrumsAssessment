...
def sliceDrums_from_annotations_SDtrainset(instrument_name, segments_dir, song_dict, fs):
    """
    ...
    for audio in song['audios']:
      ...
      for i in range(1,n_notes):
          if i != n_notes-1 and i != 0:
              x_seg = audio[(annotations[i][2]-3000 + first_onset):(annotations[i+1][2]-3000 + first_onset)] 
          ...
          if annotations[i][1] == 'N':
            continue
          filename = os.path.join(x_seg_dir, instrument_name + '_' + str(dir_n_files) + '.wav')
          ess.MonoWriter(filename = filename, format = 'wav', sampleRate = fs)(x_seg)
          file_count = file_count + 1
...
def get_wavs_in_dir(dir_name):
    ...
    for root, dirs, files in os.walk(dir_name):
        for file in files:
            if file == ".DS_Store" or "ipynb_checkpoints" in root or ".wav" not in root:
                continue
            file_name = os.path.join(root,file)
            file_names.append(file_name)
    file_names.sort()
